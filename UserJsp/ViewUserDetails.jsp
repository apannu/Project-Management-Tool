...
<head>
<title>Cisco Project Management Portal - View User Details</title>
      ...
      <div class="mainbar">
        <div class="article">
	        <fieldset><legend class="text7">Personal Details</legend>
					<form action="./UserServlet?action=UpdateUserJsp" method="post" name="ViewUserDetailsForm">	
						<table>
							<tr>
							...
							<tr>
								<c:forEach items="${list1}" var="list">	
									...
									<td class="text5"><c:if test="${empty list.emergencyContactNumber}">Not Available</c:if>${list.emergencyContactNumber}</td>
								</c:forEach>
							</tr>
							<tr>
								<c:forEach items="${list1}" var="list">	
							...
							<td><label class="text11">N+1:</label></td>
								...
								<c:forEach items="${list4}" var="list4">
									...
									<c:if test="${list1.capgeminiManagerName!=list1.username}">
										<td class="text5"><a href="./UserServlet?action=ViewUserDetailsServlet&userType=nonAdmin&username=${list1.capgeminiManagerName}">${list4.firstName} ${list4.lastName}(${list1.capgeminiManagerName})</a></td>
									</c:if>
								</c:forEach>
								</c:forEach>
							</tr>	
							...
							<td><label class="text11">N+2:</label></td>
								...
								<c:forEach items="${list6}" var="list6">
									...
									<c:if test="${list4.managerName2!=list4.username}">
										<td class="text5"><a href="./UserServlet?action=ViewUserDetailsServlet&userType=nonAdmin&username=${list4.managerName2}">${list6.firstName} ${list6.lastName}(${list4.managerName2})</a></td>
									</c:if>
								</c:forEach>
								</c:forEach>
							</tr>		
							<tr>
								<c:forEach items="${list1}" var="list">
									...
									<td class="text5"><a href="./UserServlet?action=ViewUserDetailsServlet&userType=nonAdmin&username=${list.createUserId}">${list.createUserId}</a></td>
								</c:forEach>
							</tr>
							<tr>
								<c:forEach items="${list1}" var="list">
							...
							<tr>
								<c:forEach items="${list1}" var="list">
									...
									<td class="text5"><a href="./UserServlet?action=ViewUserDetailsServlet&userType=nonAdmin&username=${list.modifyUserId}">${list.modifyUserId}</a></td>
								</c:forEach>
							</tr>
							<tr>
								<c:forEach items="${list1}" var="list">
						...
						<c:forEach items="${list1}" var="list">
							<c:forEach items="${usersList}" var="user">
								<c:if test="${user.username==list.username}">
			...
			<fieldset>
				...
				<table id="customers">
					<tr>
						...
						<c:forEach items="${list1}" var="list1"><c:if test="${list1.isBillable=='N'}">No Tagging available for ${list1.username}<a href="./UserServlet?action=TagUserJsp">(Tag User)</a></c:if></c:forEach>
					...
					<c:forEach items="${list2}" var="list2">
						<c:forEach items="${list3}" var="list3">	
								<td><a href="./UserServlet?action=ViewProjectDetailsServlet&userType=nonAdmin&projectName=${list3.projectName}">${list3.projectName}(${list2.project_code})</a></td>
								<td><a href="./UserServlet?action=ViewUserDetailsServlet&userType=nonAdmin&username=${list3.projectManager}">${list3.firstName} ${list3.lastName}(${list3.projectManager})</a></td>
						</c:forEach>
					</c:forEach>
					<c:forEach items="${list1}" var="list1">	
					<c:forEach items="${list2}" var="list2">	
